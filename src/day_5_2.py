...
class HydrothermalVentureMap:
    ...
    def add_vector_to_grid(self, start, end):
        ...
        if (start[0] != end[0]) and (start[1] != end[1]):
            x_direction = "up" if start[0] < end[0] else "down"
            ...
            while steps != (length + 1):
                self.field_grid[diagonal_x][diagonal_y] = self.field_grid[diagonal_x][diagonal_y] + 1
                diagonal_x = diagonal_x + 1 if x_direction == "up" else diagonal_x - 1
                diagonal_y = diagonal_y + 1 if y_direction == "up" else diagonal_y - 1
                steps += 1
