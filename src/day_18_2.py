...
def find_exploding_pair(snail_number):
    """
    A pair explodes when it is nested 4 levels deep into the snail number. it's always a pair.
    ...
    [[[[1, [3, 2]]]]]
    still [3,2] will explode, not 1

    :param snail_number:
    :return: the exploding pair and the index of that pair in the original snailnumber, or None, float('inf') if
        none is found
    """
...
def explode_pair(snail_number, exploding_pair, index_exploding_pair):
    """
    Explode a pair by adding the left side to the first integer left to it in the snailnumber (if any)
    by adding the right side to the first integer to the right in the snailnumber (if any)
    by replacing the original pair [left side, right side]  with a '0'

    :param snail_number:
    :param exploding_pair:
    :param index_exploding_pair:
    :return: snail number after exploding
    """
...
def requires_split(snail_number):
    """
    Check whether there are any double- or multidigit numbers and return the index of the first occurence and the
    double number as a integer

    :param snail_number:
    :return: tuple of index of first double digit in snailnumber and double digit as int, or None, None
    """
...
def split_snail_number(snail_number, split_index, split_number):
    """
    Replace a double- or multiple-digit digit with a split pair
    [double digit / 2 rounded down, double digit / 2 rounded up]

    :param snail_number:
    :param split_index:
    :param split_number:
    :return: snailnumber with the double digit replaced with a split pair
    """
...
def reduce(snail_number: str) -> str:
    """
    reduce a single snailnumber by looping through the actions, applying the first that applies:
    - exploding a pair
    - splitting a double digit into a new pair

    :param snail_number:
    :return: reduces snail number
    """
...
def reduce_sum(sub_list):
    """
    Reduce the pairs to find the magnitude
    :param sub_list:
    :return:
    """
    lhs, rhs = sub_list
...
def process(input_list: list):
    """
    Find magnitude of the total sum of the snailnumbers input
    :param input_list:
    :return:
    """
    index = 1
...
def find_largest_sum(input_list):
    """
    find pair of snailnumbers with largest magnitude
    :param input_list:
    :return:
    """
    largest_sum = 0
    for lhs_index, lhs_number in enumerate(input_list):
        for rhs_index, rhs_number in enumerate(input_list):
            if rhs_index != lhs_index:
                lhs = reduce(lhs_number)
                rhs = reduce(rhs_number)
                snail_number = f'[{lhs},{rhs}]'
                print(f"after addition: {snail_number}")
                final_sum = reduce(snail_number)
